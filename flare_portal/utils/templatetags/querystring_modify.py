...
def querystring_modify(
    ...
    When specifying key/value pairs, any keys that do not already exist in the
    base value will be added, and those that do will have their value replaced.
    Specifying a value of ``None`` for an existing item will result in it being
    discounted. For example, if the querystring for the current request were
    "foo=ORIGINALFOOVAL&bar=ORIGINALBARVAL", but you wanted to:
    ...
    * Add a new "baz" item with the value `1`
    ...
    Values can be strings, booleans, integers or references to other variables
    in the current context. For example, if the tag were being used to generate
    pagination links, where the page number was a variable named ``page_num``,
    you could reference that value like so:
    ...
    You can also specify more than one value for a key by providing an iterable
    as a value. For example, if the context contained a variable ``tag_list``,
    which was list of 'tag' values (```['tag1', 'tag2', 'tag3']```), you
    include all of those values by referencing the list value. For example:
    ...
    The output of the above would be "?tags=tag1&amp;tags=tag2&amp;tags=tag3"
    (plus whatever other values were in the base value).
    ...
    For example, if the querystring was
    "tags=tag1&amp;tags=tag2&amp;tags=tag3", and you wanted to remove 'tag2',
    you could do:
    ...
    Similarly, if you're unsure whether the value is present or not, and would
    like to remove it if it is, or add it if it is not, you can also use the
    '__toggle' option. For example:
